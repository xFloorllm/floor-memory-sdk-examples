...
export default function QueryConversation() {
  ...
  const handleSubmit = async (e) => {
    ...
    const resolvedAppId = localStorage.getItem('xfloor_app_id') || APP_CONFIG.APP_ID || '';
    const normalizedUserId = userId?.trim?.() || '';
    const normalizedFloorId = floorId?.trim?.() || '';

    if (!normalizedUserId || !normalizedFloorId || !resolvedAppId) {
      const missing = [];
      if (!normalizedUserId) missing.push('user_id');
      if (!normalizedFloorId) missing.push('floor_id');
      if (!resolvedAppId) missing.push('app_id');

      const setupError = {
        id: Date.now(),
        type: 'error',
        content: `Missing required context: ${missing.join(', ')}. Re-enter credentials and try again.`,
        timestamp: new Date().toISOString(),
      };
      setMessages(prev => [...prev, setupError]);
      return;
    }
