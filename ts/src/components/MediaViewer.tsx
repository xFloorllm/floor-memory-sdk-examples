    ...
    <div className="media-viewer-overlay" onClick={onClose}>
      <div className="media-viewer-container" onClick={(e) => e.stopPropagation()}>
        <div className="media-viewer-header">
          <div className="media-viewer-title">{getMediaTypeLabel(media.type)}</div>

          <div className="media-viewer-controls">
            {media.type === 'IMAGE' && (
              <>
                <button className="media-control-btn" onClick={handleZoomIn} title="Zoom In" disabled={zoom >= maxZoom}>
                  🔍+
                </button>
                <span className="zoom-level">{Math.round(zoom * 100)}%</span>
                <button className="media-control-btn" onClick={handleZoomOut} title="Zoom Out" disabled={zoom <= minZoom}>
                  🔍−
                </button>
                {zoom > 1 && (
                  <button className="media-control-btn" onClick={handleResetZoom} title="Reset Zoom">
                    ↺
                  </button>
                )}
              </>
            ...
            <button className="media-control-btn close-btn" onClick={onClose} title="Close">
              ✕
            </button>
          </div>
        </div>
        ...
        >
          ...
          {media.type === 'VIDEO' && (
            <video src={media.url} controls className="media-viewer-video" autoPlay>
          ...
          {media.type === 'PDF' && (
            <div className="media-viewer-pdf">
              <iframe src={media.url} className="pdf-iframe" title="PDF Viewer" />
            </div>
          )}
        </div>
