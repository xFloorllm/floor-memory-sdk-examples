...
type ActiveView = 'home' | 'query' | 'create' | 'recent' | 'notes';
...
function App() {
  ...
  const [editFloorData, setEditFloorData] = useState<EditFloorData>({
    title: '',
    details: '',
    logoFile: null,
  });
  const [editFloorLoading, setEditFloorLoading] = useState(false);
  const [editFloorError, setEditFloorError] = useState<string | null>(null);
  ...
  const handleEditFloor = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    if (!podInfo?.floorId) {
      setEditFloorError('Floor information is missing.');
      return;
    }
    ...
    try {
      const updatedFloor = await authService.editFloor({
        ...
        logoFile: editFloorData.logoFile || undefined,
      });

      updatePodInfo(updatedFloor);
      setShowEditFloorModal(false);
      setEditFloorData({ title: '', details: '', logoFile: null });
    } catch (err) {
      setEditFloorError(toErrorMessage(err, 'Failed to edit floor'));
    } finally {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setEditFloorData((prev) => ({ ...prev, logoFile: file }));
    }
  };
  ...
  return (
    <div className={`app main-app ${theme}-theme`}>
            ...
            <div className="profile-menu" ref={profileMenuRef}>
              ...
              >
                {user?.profile?.avatar?.url ? (
                  ...
                  <span className="profile-avatar-placeholder">{user?.profile?.name?.[0]?.toUpperCase() || '👤'}</span>
                )}
              </button>
              {showProfileDropdown && (
        ...
        <div className="floors-container">
          <div className="floors-scroll">
            <div className="floor-card active">
              ...
              <button className="edit-floor-btn" onClick={() => setShowEditFloorModal(true)} title="Edit floor">
                ✏️
              </button>
            </div>
          </div>
      ...
      <footer className="app-footer">
        <p>
          Built with{' '}
          <a href="https://docs.xfloor.ai/typescript-sdk-1930595m0" target="_blank" rel="noopener noreferrer">
            xFloor TypeScript SDK
          </a>
        </p>
      </footer>
        ...
        <div className="modal-overlay" onClick={() => setShowEditFloorModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowEditFloorModal(false)}>
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleEditFloor} className="edit-floor-form">
              ...
              <div className="form-group">
                ...
                <input id="floor-logo" type="file" accept="image/*" onChange={handleFileChange} />
                {editFloorData.logoFile && <div className="file-preview">Selected: {editFloorData.logoFile.name}</div>}
              </div>

              {editFloorError && <div className="auth-error">{editFloorError}</div>}
              ...
              <div className="modal-actions">
                ...
                <button
                  ...
                  disabled={editFloorLoading || (!editFloorData.title && !editFloorData.details && !editFloorData.logoFile)}
                >
